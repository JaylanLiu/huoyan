...
def get_day_item_num(
    df: pd.DataFrame, day: str, item: str, organization: str = "total"
):
    if organization != "total":
        ndf = df[
            (df[item].str.contains(day)).fillna(False)
                                    ...
                                    [
                                        dcc.Dropdown(
                                            ...
                                            options=[
                                                ...
                                                {
                                                    "label": "哈尔滨市道外区疾病预防控制中心",
                                                    "value": "哈尔滨市道外区疾病预防控制中心",
                                                },
                                                {
                                                    "label": "道外区大有社区卫生服务中心",
                                                    "value": "道外区大有社区卫生服务中心",
                                                ...
                                                {
                                                    "label": "哈尔滨广积德中医医院有限公司",
                                                    "value": "哈尔滨广积德中医医院有限公司",
                                                },
                                                {
                                                    "label": "哈尔滨三芝堂中医门诊部",
                                                    "value": "哈尔滨三芝堂中医门诊部",
                                                ...
                                                {
                                                    "label": "国家税务总局哈尔滨市阿城区税务局",
                                                    "value": "国家税务总局哈尔滨市阿城区税务局",
                                                },
                                                {
                                                    "label": "哈尔滨市双城区教育局",
                                                    "value": "哈尔滨市双城区教育局",
...
def dropdown_options(org):
    global df
    last_10_days = pd.DatetimeIndex(
        end=time.strftime("%Y-%m-%d", time.localtime()), freq="d", periods=10
    )
    fig = go.Figure()
    ...
    for item, tag in zip(["test", "extract", "report"], ["到样数", "检测数", "报告数"]):
        fig.add_trace(
            go.Bar(
                x=days,
                y=[
                    get_day_item_num(df, day, organization=org, item=item)
                    for day in days
                ],
                name=tag,
            )
...
def process_info(contents, filename):
    ...
    try:
        ...
        ndf = pd.read_csv(
            io.StringIO(decoded.decode("utf-8")), dtype={"证件号码": "str", "电话": "str"}
        )
        # print(ndf) #该处也正常
...
def process_qPCR(contents, filename):
    ...
    try:
        ...
        csv_string = "data:text/csv;charset=gb2312,\ufeff" + urllib.parse.quote(
            csv_string
        )
