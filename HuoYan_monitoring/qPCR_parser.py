    ...
    for xjsj_f, xjsj_n in zip(xjsj_ns, xjsj_fs):
        ...
        def find_qPCR(xjsj_f: str):
            ...
            return [
                target
                for target in dts
                if os.path.basename(xjsj_f).split(".")[0] in target
            ][0]
        ...
        def find_tqrwd(xjsj_f: str):
            ...
            return [
                ...
                if "-".join(
                    [
                        seg
                        for seg in os.path.basename(xjsj_f).split(".")[0].split("-")
                        if seg != "RQ"
                    ][:2]
                )
                in target
        ...
        tdf.loc[tdf[tdf.样本名称 == "空白质控1"].index, "VIC_判定"] = tdf[
            tdf.样本名称 == "空白质控1"
        ].apply(lambda x: vic_blank_jundge(x.VIC), axis=1)

        tdf.loc[tdf[tdf.样本名称 == "空白质控2"].index, "VIC_判定"] = tdf[
            tdf.样本名称 == "空白质控2"
        ].apply(lambda x: vic_blank_jundge(x.VIC), axis=1)

        tdf.loc[tdf[tdf.样本名称 == "空白质控3"].index, "VIC_判定"] = tdf[
            tdf.样本名称 == "空白质控3"
        ].apply(lambda x: vic_blank_jundge(x.VIC), axis=1)
        ...
        tdf.loc[tdf[tdf.样本名称 == "阳性质控"].index, "VIC_判定"] = tdf[
            tdf.样本名称 == "阳性质控"
        ].apply(lambda x: vic_posi_jundge(x.VIC), axis=1)
        ...
        tdf.loc[tdf[tdf.样本名称 == "阳性质控"].index, "FAM_判定"] = tdf[
            tdf.样本名称 == "阳性质控"
        ].apply(lambda x: fam_posi_jundge(x["FAM Ct值"]), axis=1)
        ...
        def combine_judge(
            VIC_judge: str, FAM_judge: str, VIC_whole_judge: str, FAM_whole_judge: str
        ):
        ...
        result_and_remark = tdf.apply(
            lambda x: combine_judge(
                x["VIC_判定"], x["FAM_判定"], x["VIC_整版判定"], x["FAM_整版判定"]
            ),
            axis=1,
        )
        ...
        positive_control_fam_ct = tdf.loc[
            tdf[tdf.样本名称 == "阳性质控"].index, "FAM Ct值"
        ].iloc[0]
